...
const bodyParser = require('body-parser'); /*importation du package de traitement de requête JSON*/
const multer = require('multer'); /*importation du package de traitement des fichiers joints*/
const fs = require('fs'); /*importation du package d'interaction fichier (pour suppression dans notre cas)*/
...
exports.createANewSauce = async (req, res, next) => {
  ...
  newSauce.save()
    ...
    .catch((error) => {
      res.status(400).json({ error }); /*Erreur dans la sauvegarder de la nouvelle sauce*/
    });
};
...
exports.getAllSauces = (req, res, next) => {
  ...
  .catch((error) => res.status(404).json({ error })); /*Erreur de traitement de la requete de recherche de sauce dans la database*/
};
...
exports.deleteOneSauce = (req, res, next) => {
  ...
  .then((sauce) => {
    if (req.auth.userId === sauce.userId) {
      ...
      fs.unlink(`images/${filename}`, ()=>{ /*Suppression de l'image dans la database*/
